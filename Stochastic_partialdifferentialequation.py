...
####Only for this part did I use some AI to make time deadline. Writing the stochastic differnetial equation solver via MLE by hand takes a long time, but its fairly straight forward.
#My Goal is to assume the price process follows a mean reverting process. I estimate the parameters using MLE, then forecast not only the future prices, but a theoretical
##forward distribution. In this case its normal distributed, because I only used a simple model. In the literature there are jump models that give better skewness.
...
# -----------------------------------------------------------
# 1) Fit OU process to price series (It assumes normality but this is just an exercise, there are more advanced models with poisson jumps)
# -----------------------------------------------------------
...
# -----------------------------------------------------------
# 3) Fit, simulate, and plot *Simulate many different paths, then we aggregate them to form distribution
# -----------------------------------------------------------
